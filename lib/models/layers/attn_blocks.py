...
def candidate_elimination(attn: torch.Tensor, tokens: torch.Tensor, lens_t: int, keep_ratio: float, global_index: torch.Tensor, box_mask_z: torch.Tensor):
    ...
    if box_mask_z is not None:    # 筛选的只有中间点
...
# 应用于moe的候选消除
def candidate_elimination_moe(attn: torch.Tensor, tokens: torch.Tensor, lens_t: int, keep_ratio: float, global_index: torch.Tensor, box_mask_z: torch.Tensor):
    """
    Eliminate potential background candidates for computation reduction and noise cancellation.
    Args:
        attn (torch.Tensor): [B, num_heads, L_t + L_s, L_t + L_s], attention weights
...
# 应用于moe的候选消除
def candidate_elimination_moelan(attn: torch.Tensor, tokens: torch.Tensor, lens_t: int, keep_ratio: float, global_index: torch.Tensor, box_mask_z: torch.Tensor):
    """
    Eliminate potential background candidates for computation reduction and noise cancellation.
    Args:
        attn (torch.Tensor): [B, num_heads, L_t + L_s, L_t + L_s], attention weights
    ...
    """
    # lens_t_all = 40
    lens_s = attn.shape[-1] - lens_t     # 减去两个模板长度
    bs, hn, _, _ = attn.shape
    ...
    attn_t = attn[:, :, :lens_t, lens_t:]   # 第一个模板和search的attention
    # attn_t2 = attn[:, :, lens_t:lens_t_all, lens_t_all:]   # 第二个模板
...
class CEMOEBlock(nn.Module):
    ...
    def forward(self, x, global_index_template, global_index_search, mask=None, ce_template_mask=None, keep_ratio_search=None):
        ...
        if self.keep_ratio_search < 1 and (keep_ratio_search is None or keep_ratio_search < 1):
            keep_ratio_search = self.keep_ratio_search if keep_ratio_search is None else keep_ratio_search
            x, global_index_search, removed_index_search = candidate_elimination_moe(attn, x, lens_t, keep_ratio_search, global_index_search, ce_template_mask)
...
class CEMOELANBlock(nn.Module):
    ...
    def forward(self, x, global_index_template, global_index_search, mask=None, ce_template_mask=None, keep_ratio_search=None):
        ...
        if self.keep_ratio_search < 1 and (keep_ratio_search is None or keep_ratio_search < 1):
            keep_ratio_search = self.keep_ratio_search if keep_ratio_search is None else keep_ratio_search
            x, global_index_search, removed_index_search = candidate_elimination_moelan(attn, x, lens_t, keep_ratio_search, global_index_search, ce_template_mask)
...
class CEMDOTBlock(nn.Module):
    ...
    def forward(self, x, global_index_template, global_index_search, mask=None, ce_template_mask=None, keep_ratio_search=None):
        ...
        if self.keep_ratio_search < 1 and (keep_ratio_search is None or keep_ratio_search < 1):
            keep_ratio_search = self.keep_ratio_search if keep_ratio_search is None else keep_ratio_search
            x, global_index_search, removed_index_search = candidate_elimination_mdot(attn, x, lens_t, keep_ratio_search, global_index_search, ce_template_mask)
...
#  ThreeMDOT的Block
class CEThreeMDOTBlock(nn.Module):
...
# 应用于three mdot的候选消除
def candidate_elimination_mdot_three(attn: torch.Tensor, tokens: torch.Tensor, lens_t: int, keep_ratio: float, global_index: torch.Tensor, box_mask_z: torch.Tensor):
    """
    Eliminate potential background candidates for computation reduction and noise cancellation.
    Args:
        attn (torch.Tensor): [B, num_heads, L_t + L_s, L_t + L_s], attention weights
...
# 应用于mdot的候选消除
def candidate_elimination_mdot2s(attn: torch.Tensor, tokens: torch.Tensor, lens_t: int, lens_x: int, keep_ratio: float, global_index: torch.Tensor, box_mask_z: torch.Tensor):
    """
    Eliminate potential background candidates for computation reduction and noise cancellation.
    Args:
        attn (torch.Tensor): [B, num_heads, L_t + L_s, L_t + L_s], attention weights
    ...
    lens_t_all = 2*lens_t
    len_search2 = lens_x
    lens_s = attn.shape[-1] - lens_t_all - len_search2     # 减去两个模板长度，剩两个搜索的长度, 再减一个
    bs, hn, _, _ = attn.shape
    ...
    attn_t = attn[:, :, :lens_t, (lens_t_all+len_search2):]   # 第一个模板和search的attention
    attn_t2 = attn[:, :, lens_t:lens_t_all, (lens_t_all+len_search2):]   # 第二个模板
    ...
    # separate template and search tokens
    tokens_t = tokens[:, :(lens_t_all+len_search2)]           # 两个模板+1个搜索
    tokens_s = tokens[:, (lens_t_all+len_search2):]
...
class CEMDOT2SBlock(nn.Module):
    ...
    def forward(self, x, lens_x, global_index_template, global_index_search, mask=None, ce_template_mask=None, keep_ratio_search=None):
        x_attn, attn = self.attn(self.norm1(x), mask, True)
        x = x + self.drop_path(x_attn)
        lens_t = global_index_template.shape[1]
        ...
        if self.keep_ratio_search < 1 and (keep_ratio_search is None or keep_ratio_search < 1):
            keep_ratio_search = self.keep_ratio_search if keep_ratio_search is None else keep_ratio_search
            x, global_index_search, removed_index_search = candidate_elimination_mdot2s(attn, x, lens_t,lens_x, keep_ratio_search, global_index_search, ce_template_mask)
