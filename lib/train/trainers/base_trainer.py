...
class BaseTrainer:
        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    if epoch > (max_epochs - 1) or save_every_epoch or epoch % 2 == 0 or epoch in save_epochs or epoch > (max_epochs - 5):
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        #assert net_type == checkpoint_dict['net_type'], 'Network is not of correct type.'            ########### 这玩意直接注释了不知道有没有用
        ...
        for key in fields:
            if key in ignore_fields:
                continue
            if key == 'net':
                net.load_state_dict(checkpoint_dict[key],strict=False)       # 这两个strict都设置成false
            elif key == 'optimizer':
