...
class TrackingSamplerMDOT(torch.utils.data.Dataset):
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            seq_id2, visible2, seq_info_dict2 = self.sample_seq_from_dataset_mdot(dataset, is_video_dataset, seq_id)     # 选取对应的sequence
            ...
            try:
                ...
                template_frames2, template_anno2, meta_obj_train2 = dataset.get_frames(seq_id2, template_frame_ids, seq_info_dict2)

                template_frames = template_frames + template_frames2   # 两个模板加一起

                template_anno['bbox'] = template_anno['bbox'] + template_anno2['bbox']

                H, W, _ = template_frames[0].shape
                template_masks = template_anno['mask'] if 'mask' in template_anno else [torch.zeros((H, W))] * self.num_template_frames*2      # 双机翻两倍
    ...
    # 得到另一机
    def sample_seq_from_dataset_mdot(self, dataset, is_video_dataset, seq_id):

        seq_cls = dataset.sequence_list[seq_id][:-2]       # 'md20xx'

        seq_cls_list = dataset.seq_per_class[seq_cls]

        tmp_index = seq_cls_list.index(seq_id)

        if tmp_index == 0:
            seq_id2 = seq_cls_list[1]
        elif tmp_index == 1:
            seq_id2 = seq_cls_list[0]
        else:
            return ValueError("无效的无人机id")

        # Sample a sequence with enough visible frames
            
        # Sample frames
        seq_info_dict2 = dataset.get_sequence_info(seq_id2)
        visible2 = seq_info_dict2['visible']



        return seq_id2, visible2, seq_info_dict2
