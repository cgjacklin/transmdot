...
class Sequence:
    ...
    def __init__(self, name, frames, dataset, ground_truth_rect, ground_truth_seg=None, init_data=None,
                 object_class=None, target_visible=None, object_ids=None, multiobj_mode=False, nlpPath=None):
        self.name = name
        self.frames = frames
    ...
    def _construct_init_data(self, init_data):
        if init_data is not None:
            ...
            if self.nlpPath is not None:
                init_data[0]['nlp'] = self.nlp_info(self.nlpPath)
    ...
    def init_info(self):
        ...
        # if self.nlpPath is not None:
        #     info['nlp'] = self.nlp_info(self.nlpPath)
        return info
    ...
    def nlp_info(self, nlpPath):   # 读取nlp信息
        with open(nlpPath, encoding='utf-8') as file:
            nlp = file.read()
        return nlp
