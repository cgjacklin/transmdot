...
from lib.models.moe import build_moetrack
...
class MoETrack(BaseTracker):
    def __init__(self, params, dataset_name):
        super(MoETrack, self).__init__(params)
        network = build_moetrack(params.cfg, training=False)
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            ...
            nl_token_masks=torch.tensor(search_nl_token_masks).cuda().unsqueeze(1)

            out_dict = self.network.forward(
                template=self.z_dict1.tensors, nl_token_ids=nl_token_ids,nl_token_masks=nl_token_masks,search=x_dict.tensors)
