...
from lib.models.ostrack import build_ostrack_three
...
class Threemdot(BaseTracker):
    def __init__(self, params, dataset_name):
        super(Threemdot, self).__init__(params)
        network = build_ostrack_three(params.cfg, training=False)
    ...
    #   三机初始化
    def three_multi_initialize(self, image_a, image_b, image_c, init_info_a, init_info_b, init_info_c):
        ...
        # C机模板
        z_patch_arr3, resize_factor3, z_amask_arr3 = sample_target(image_c, init_info_c['init_bbox'], self.params.template_factor,   # 对(720,1280,3)的image进行裁剪
                                                    output_sz=self.params.template_size)
        self.z_patch_arr3 = z_patch_arr3
        template3 = self.preprocessor.process(z_patch_arr3, z_amask_arr3)
        with torch.no_grad():
            self.z_dict3 = template3

        self.box_mask_z3 = None
        if self.cfg.MODEL.BACKBONE.CE_LOC:          # 候选消除模块分别插入Vit的第几层
            template_bbox3 = self.transform_bbox_to_crop(init_info_c['init_bbox'], resize_factor3,
                                                        template.tensors.device).squeeze(1)
            self.box_mask_z3 = generate_mask_cond(self.cfg, 1, template.tensors.device, template_bbox3)
...
# general 重检测
    ...
    # 三机融合，搜索A的搜索区域，融合ABC模板，主要用这个
    def three_multi_Fusetrack(self, image_a, image_b, image_c, drone_id, info_a: dict = None, info_b: dict = None, info_c: dict = None):          # self.state是上帧追踪结果，就是一个框
        H, W, _ = image_a.shape
        self.frame_id += 1
        x_patch_arr, resize_factor, x_amask_arr = sample_target(image_a, self.state, self.params.search_factor,
                                                                output_sz=self.params.search_size)  # (x1, y1, w, h)     # 以目标框为中心划定A的搜索区域
        ...
        with torch.no_grad():
            ...
            # run the transformer
            out_dict = self.network.forward(                  # 在forward这里要把两个模板传进去
                template=self.z_dict1.tensors, template2=self.z_dict2.tensors,template3=self.z_dict3.tensors, search=x_dict.tensors, ce_template_mask=self.box_mask_z)
...
# 将B机搜索区域在A机中查询，然后重检测
    def three_search_redetect(self, image_a, image_b, drone_id, state_b, tmp_factor = 4.0, tmp_s_factor = 12.0, info_a: dict = None, info_b: dict = None):       
        ...
        with torch.no_grad():
            ...
            # run the transformer
            out_dict = self.network.forward(                  # 在forward这里要把两个模板传进去
                template=search_b.tensors, template2=search_b.tensors, template3=search_b.tensors, search=search_a.tensors, ce_template_mask=self.box_mask_z)
        ...
        self.frame_id -= 1
        #out_t, max_score_t, response_APCE_t = self.three_multi_Fusetrack(image_a, image_b, image_b, drone_id, info_a, info_b, info_b)
        ...
        if tmp_s_factor > 8:
            ...
            with torch.no_grad():
                out_dict_ctr = self.network.forward(                 
                    template=search_b.tensors, template2=search_b.tensors, template3=search_b.tensors, search=search_ctr.tensors, ce_template_mask=self.box_mask_z)   # 在中间用搜索区域检测
            pred_score_map_ctr = out_dict_ctr['score_map']
            pred_boxes_ctr, max_score_ctr = self.network.box_head.cal_bbox(pred_score_map_ctr, out_dict_ctr['size_map'], out_dict_ctr['offset_map'], return_score=True)   # 获得最大score
    ...
    # 三机无多模板但是有重检测
    def three_nomulti_Fusetrack(self, image_a, image_b, image_c, drone_id, info_a: dict = None, info_b: dict = None, info_c: dict = None):          # self.state是上帧追踪结果，就是一个框
        H, W, _ = image_a.shape
        self.frame_id += 1
        x_patch_arr, resize_factor, x_amask_arr = sample_target(image_a, self.state, self.params.search_factor,
                                                                output_sz=self.params.search_size)  # (x1, y1, w, h)     # 以目标框为中心划定A的搜索区域
        ...
        with torch.no_grad():
            ...
            # run the transformer
            out_dict = self.network.forward(                  # 在forward这里要把两个模板传进去
                template=self.z_dict1.tensors, template2=self.z_dict1.tensors,template3=self.z_dict1.tensors, search=x_dict.tensors, ce_template_mask=self.box_mask_z)
